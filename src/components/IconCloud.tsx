...
export function IconCloud({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    iconPositions.forEach((icon) => {
      ...
      const clickRadius = (iconSize / 2) * scale;
      const dx = x - screenX;
      const dy = y - screenY;
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iconPositions.forEach((icon, index) => {
        ...
        const scale = (rotatedZ + 300) / 450;
        const opacity = Math.max(0.3, Math.min(1, (rotatedZ + 200) / 300));
  ...
  }, [icons, iconPositions, isDragging, mousePos, targetRotation, iconSize]);
